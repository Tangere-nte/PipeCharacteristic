...
class SummarizeResult1:
    ...
    def __init__(self, **kwargs) -> object:
        for key, (pressure_value, flow_value) in kwargs.items():
            setattr(
                self, key, self.SubObject(pressures=pressure_value, flows=flow_value)
            )
...
def calculate_flow_from_sources(
    ...
    Args:
        ...
        edge_indices (LArray2D) : indices of every init nodes and paired target of that flow,
...
def calculate_p_flow(
    ...
    and also calculate the pressure of sources and junctions from the pressures
    of sinks according to govern equation
    ...
    Args:
        ...
        bs (FArray)             : set of the pipe Characteristic of each flow in Pascal
                                    second^k per kilogram^k.
    ...
    Constant args:
        ...
        K (float)                   : constant exponential of flow to determine the
                                        effect to friction (dimensionless variable).

    Returns:
        SummarizeResult1   : a object that collects a set of every node's pressure
                                and a set of every pipe's flow
    ...
    for edge_idx in np.arange(num_edges - 1, -1, -1):
        ...
        ps[idx_init] = (
            ps[idx_target]
            + DENSITY * GRAVITATIONAL_ACCEL * (heights[idx_target] - heights[idx_init])
...
def calculate_pipe_characteristic(
    ...
    for edge_idx in np.arange(num_edges):
        ...
        bs[edge_idx] = (
            p_s[idx_target]
            - p_s[idx_init]
            - DENSITY * GRAVITATIONAL_ACCEL * (heights[idx_init] - heights[idx_target])
        ) / (flows[idx_init]) ** K
